...
void D::SetupFonts()
{
	I::Surface->SetFontGlyphSet(F::Meme = I::Surface->Create_Font(), charenc("Tahoma"), 14, FW_DONTCARE, NULL, NULL, FONTFLAG_ANTIALIAS);
	I::Surface->SetFontGlyphSet(F::ESP = I::Surface->Create_Font(), charenc("Tahoma"), 16, FW_DONTCARE, NULL, NULL, FONTFLAG_OUTLINE);
	I::Surface->SetFontGlyphSet(F::Watermark = I::Surface->Create_Font(), charenc("Tahoma"), 18, FW_DONTCARE, NULL, NULL, FONTFLAG_ANTIALIAS);
}
...
void D::ShowRadar(float X, float Y, float Size)
{
	...
	if (Vars.Visuals.Radar.Range < 5)
		Vars.Visuals.Radar.Range = 5;

	if (Vars.Visuals.Radar.Size < 75)
		Vars.Visuals.Radar.Size = 75;

	if (Vars.Visuals.Radar.X < 100)
		Vars.Visuals.Radar.X = 100;

	if (Vars.Visuals.Radar.Y < 100)
		Vars.Visuals.Radar.Y = 100;
	...
	Border(centerx - Size - 6, centery - Size - 6, 2 * Size + 6, 2 * Size + 6, 6, Color(174, 56, 69, 255)); //all border
	DrawRect(centerx - Size, centery - Size, 2 * Size, 2 * Size, Color(37, 39, 46, 220));
	//DrawRect(centerx, centery - Size, 1, 2 * Size, Color(mColor[0] * 255.0f, mColor[1] * 255.0f, mColor[2] * 255.0f, 255));
	//DrawRect(centerx - Size, centery, 2 * Size, 1, Color(mColor[0] * 255.0f, mColor[1] * 255.0f, mColor[2] * 255.0f, 255));
	...
	Border(centerx - Size - 6, centery - Size - 6, 2 * Size + 11, 2 * Size + 11, 1, Color(mColor[0] * 255.0f, mColor[1] * 255.0f, mColor[2] * 255.0f, 255));
...
void D::DrawRadarEntity(float X, float Y, float RadarSize, Color _Color, Vector RadarX, Vector RadarY, QAngle Angle, int HP, bool IsFriend)
{
	...
	DrawRect(ScreenX - 2, ScreenY - 2, 4, 4, Color(150, 0, 0, 255));

		DrawOutlinedRect(ScreenX - 4, ScreenY + 4, HP * 8 / 100, 1, _Color);
	}